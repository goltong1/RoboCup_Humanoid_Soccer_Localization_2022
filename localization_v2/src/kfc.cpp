//---------------------------------------------------------------------------
#include "../include/localization_v2/kfc.h"
...
KVector KMatrix::Cut(int row, const KList<short>& lCol) const
{
    KVector vOut(lCol.Count());

    for(int i=0; i<lCol.Count(); i++)
        vOut._ppA[i][0] = _ppA[row][lCol[i]];

    return vOut;
}

KVector KMatrix::Cut(const KList<short>& lRow, int col) const
{
    KVector vOut(lRow.Count());

    for(int i=0; i<lRow.Count(); i++)
        vOut._ppA[i][0] = _ppA[lRow[i]][col];

    return vOut;
}

KVector KMatrix::Cut(int idx, int nType) const
{
    int     nSize;
    KVector vOut;
