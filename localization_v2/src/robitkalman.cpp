#include "../include/localization_v2/robitkalman.h"
...
KalmanFilterC::KalmanFilterC(Point2f pt)
{
    ...
    setIdentity(kalman->processNoiseCov, Scalar::all(1e-8));
    setIdentity(kalman->measurementNoiseCov, Scalar::all(1e-9));
    setIdentity(kalman->errorCovPost, Scalar::all(.1));

}
void KalmanFilterC::init(Point2f pt1)
{
    kalman = new KalmanFilter(6, 2, 0);
    Mat processNoise(6, 1, CV_32F);

    kalman->statePre.at<float>(0) = pt1.x;
    kalman->statePre.at<float>(1) = pt1.y;
    kalman->statePre.at<float>(2) = 0;
    kalman->statePre.at<float>(3) = 0;
    kalman->statePre.at<float>(4) = 0;
    kalman->statePre.at<float>(5) = 0;
